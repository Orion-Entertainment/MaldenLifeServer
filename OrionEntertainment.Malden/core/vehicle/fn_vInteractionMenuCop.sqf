...
if (!dialog) then {
    createDialog "vInteraction_MenuCopVeh";
};
...
if((FETCH_CONST(life_coplevel) > 3 ) && !(_curTarget isKindOf "Cargo_base_F")) then {
    _Btn6 buttonSetAction "[] spawn life_fnc_CrushAction; closeDialog 0;";
};

if (_curTarget isKindOf "Ship") then {
    _Btn7 buttonSetAction "[] spawn life_fnc_pushObject; closeDialog 0;";
    if (_curTarget isKindOf "Ship" && {local _curTarget} && {crew _curTarget isEqualTo []}) then { _Btn7 ctrlEnable true;} else {_Btn7 ctrlEnable false};
} else {
    if (!isNil "_id") then {
        if !(_id in getDLCs 1) then {
            _Btn7 buttonSetAction "player moveInDriver life_vInact_curTarget; closeDialog 0;";
            if (crew _curTarget isEqualTo [] && {canMove _curTarget} && {locked _curTarget isEqualTo 0}) then {_Btn7 ctrlEnable true;} else {_Btn7 ctrlEnable false};
        };
    } else {
        ...
        if (alive _curTarget && {crew _curTarget isEqualTo []} && {canMove _curTarget}) then { _Btn7 ctrlEnable false;} else {_Btn7 ctrlEnable true;};
    };
};
