...
if (count _invs > 0) then {
    ...
    if (_illegal > 6000) then {
        ...
        if (life_HC_isActive) then {
            [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"485"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
        } else {
            [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"485"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
        };
