...
fnc_phone_next_page = {
	...
	_ctrls = allControls _display;
	{
		...
		if(ctrlClassName _x == "RscInvisibleButton" || ctrlClassName _x == "RscPicture" && ctrlIDC _x != 1623) then {_x ctrlShow false; _x ctrlCommit 0;};
	}  forEach _ctrls;
	...
	for [{_i=0}, {_i<_gridHeight}, {_i= _i + 1}] do {
		for [{_j=0}, {_j<_gridWidth}, {_j=_j+1}] do {
			...
			if(call compile (_currentConfig select 4)) then {
				...
				_button = _display ctrlCreate ["RscInvisibleButton", -1];
