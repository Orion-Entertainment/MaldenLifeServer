...
if (!_isVehicle) then {
    ...
    if (_dice < 30) then {
        ...
        if (life_HC_isActive) then {
            [getPlayerUID player,profileName,"216"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
        } else {
            [getPlayerUID player,profileName,"216"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
        };
