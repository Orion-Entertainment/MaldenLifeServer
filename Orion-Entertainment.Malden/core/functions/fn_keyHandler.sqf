...
switch (_code) do {
	...
	case 34: {
		if(_shift) then {_handled = true;};
		if (_shift && playerSide isEqualTo civilian  || playerSide isEqualTo west && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
